...
def current_user():
    # Pretend this came from a JWT auth dependency.
...
def get_tenant_settings(tenant_id: str, user=Depends(current_user)):
    # Vulnerable fixture: authenticated user can read any tenant by path ID.
    return db.fetch_one(
...
def promote_admin(tenant_id: str, user_id: str, user=Depends(current_user)):
    # Vulnerable fixture: only checks login, not owner role in the target tenant.
    db.execute(
