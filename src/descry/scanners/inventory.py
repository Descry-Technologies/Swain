...
_AUTH_PATTERNS = [
    r"/api/auth", r"/login", r"/signup", r"/register", r"useAuth", r"withAuth",
    r"middleware.*auth", r"jwt", r"session", r"oauth",
]

_PAYMENT_PATTERNS = [r"stripe", r"checkout", r"subscription", r"billing", r"invoice"]
...
class RepoInventory:
    ...
    def _collect_files(self) -> None:
        ignore = {".git", "node_modules", "__pycache__", ".venv", "dist", "build", "target", ".next"}
        self.all_files = [
            ...
            for f in self.all_files:
                if f.name == fname:
                    try:
                        ...
                        found.update(m.group("name") for m in re.finditer(pattern, text, re.MULTILINE))
                    except Exception:
                        pass
    ...
    def _detect_frameworks(self) -> None:
        ...
        if any(d in dep_set for d in ["pg", "postgres", "postgresql", "@supabase/supabase-js"]):
    ...
    def _detect_surfaces(self) -> None:
        ...
        for f in self.all_files:
            if f.suffix in {".ts", ".tsx", ".js", ".jsx", ".py"}:
                try:
                    ...
                    if any(re.search(p, f.read_text(errors="ignore"), re.IGNORECASE) for p in _AUTH_PATTERNS):
        ...
        self.has_payments = bool(re.search("|".join(_PAYMENT_PATTERNS), combined))
        self.has_llm_features = bool(re.search("|".join(_LLM_PATTERNS), combined))
        self.has_file_upload = bool(re.search(r"multer|formdata|upload|file.*input", combined))

        # Route files: Next.js app router, API routes
        self.route_files = [
            f for f in self.all_files
            if "app/" in str(f) and f.name in ("route.ts", "route.js", "page.tsx", "page.jsx")
    ...
    def _detect_risk_events(self, prev_deps: list[str]) -> None:
        ...
        for d in new_deps:
            if any(s in d.lower() for s in ["auth", "jwt", "session", "oauth", "clerk"]):
